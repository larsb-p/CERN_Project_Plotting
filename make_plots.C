...
class plot_maker {
	...
	std::vector<TGraph*> graphs;
		...
		for (const auto& pair : fVariableInfo) {
			...
			// There are no corrections for values of temperature and pressure sensors -> skip these
...
//	plots_map.cbegin()->second.at(0)->Draw("AP"); // Check
	...
	if ( temp_max > max && temp_max < 20000 ) max = temp_max;
	...
	plots_map.cbegin()->second.at(0)->GetYaxis()->SetRangeUser( min - 30, max + 160);
